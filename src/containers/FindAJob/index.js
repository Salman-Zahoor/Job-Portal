...
const FindAJob = () => {
  const { user, selectedCoverLetter, selectedTemplate } =
  ...
  //         } catch (error) {
  //             console.log(error,"errrrrrrrrrooooooooooo");
  ...
  const handleApplyJob = async (jobId, companyId) => {
    ...
    if (!user) {
      ...
      fetch("https://fypflaskbackend.onrender.com/analyze_resume_and_job", {
        ...
        .then((data) => {
          console.log(data, "sundas");
          if (data?.Score <= 50) {
            enqueueSnackbar(
              `Dear Candiditate your score is ${data?.Score}, therefore,  You are not eligible  for this job`,
              {
                variant: "error",
                anchorOrigin: {
                  vertical: "bottom",
            ...
            enqueueSnackbar(
              `Congratulation your score is ${data?.Score}, therefore, you have successfully applied to this job`,
              {
                variant: "success",
                anchorOrigin: {
                  vertical: "bottom",
            ...
            setResults("");
            handleApplyJobData(data.Score, user?._id, jobId, companyId);
          }
  ...
  const handleApplyJobData = async (score, userId, jobId, companyId) => {
    let payload = {
      score,
      userId,
      jobId,
    ...
    console.log("xyz12", payload);
    const result = await applyJobsFlask(payload);
    console.log("abc12", result);
