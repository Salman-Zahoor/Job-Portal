...
export default function Header(props) {
  ...
  console.log(location.pathname,"kdjkjdkjdjkjdkjkdjjkd");
  return (
    <Box sx={{ display: 'flex' }}>
      <ScrollToHide01>
                ...
                <Box sx={{ display: { xs: 'none', sm: 'block' }, marginLeft: "40px" }}>
                  {
                    navItems.map((item) => (
                      <Button onClick={() => navigate(`${item.path}`)} key={item} sx={{
                       ...
                       color: location.pathname === item.path ? primaryColor : textColor, fontSize: "14px", fontWeight: "600", padding: "6px 14px", "&:hover": {
                          color: primaryColor,
                        }
                      }}>
                        {item.name}
